...
public class Character_Stat : MonoBehaviour
{
    ...
    public virtual void DoDamage(Character_Stat target)
    {
     ...
     public virtual void DoMagicDamage(Character_Stat target)
    {
        if (TargetCanAvoidAttack())
        {
            return;
    ...
    public void CaculateMagic(Character_Stat target)
    {
    ...
    private  float CheckTargetArmor(Character_Stat target, float totalDamage)
    {   
        if(isIce){
    ...
    private void CaculateAttack(Character_Stat target)
    {
    ...
    public bool CritChance(Character_Stat target){
    ...
    private void SetFireDamage(Character_Stat target) => fireDamage = target.ignite.GetValue() * 0.2f;
